...
CHECKPOINT_FILE = os.path.join(SCRIPT_DIR, f"checkpoint_{TOPIC}.txt")

print(f"--- AUTOMATION: Target topic: {TOPIC} ---")
...
def fetch_news_chunk(topic, start_time, end_time):
    ...
    url = (f"https://www.alphavantage.co/query?function=NEWS_SENTIMENT"
       f"&topics={topic}&time_from={start_str}&time_to={end_str}"
        ...
        if count > 0:       
            ...
            if count >= 1000:
                ...
                if duration > timedelta(minutes=30): # Avoid infinite loops on extremely high volume
                    ...
                    time.sleep(15)
                    fetch_news_chunk(topic, start_time, mid_point)
                    time.sleep(15) # Cooldown
                    fetch_news_chunk(topic, mid_point, end_time)
                else:
...
for i in range(25): 
    ...
    print(f"Request {i+1}/25 | {TOPIC} | {t_from} to {t_to}")
    
    success_bool = fetch_news_chunk(TOPIC, current_pointer, window_end)
...
print(f"\nSession finished. Current progress for {TOPIC}: {current_pointer}")